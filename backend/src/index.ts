...
app.get(`${api}/admin/users`, requireUser, requireAdmin, asyncRoute(async (request, response) => { const { page, limit } = pageQuery(request); const [users, total] = await prisma.$transaction([prisma.user.findMany({ select: { id: true, email: true, role: true, createdAt: true, subscription: true }, orderBy: { createdAt: "desc" }, skip: (page - 1) * limit, take: limit }), prisma.user.count()]); return response.json({ users, pagination: { page, limit, total, pages: Math.ceil(total / limit) } }); }));
